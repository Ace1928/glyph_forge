    ...
    if border_type == "custom" and "border_chars" in style:
        ...
        if len(chars) > 0:
            ...
            lines = (
                [top]
                + [
                    f"{border_char} {line.ljust(max_line_length)} {border_char}"
                    for line in lines
                ]
                + [bottom]
            )
        ...
        lines = (
            [top]
            + [
                f"{border['vertical']} {line.ljust(max_line_length)} {border['vertical']}"
                for line in lines
            ]
            + [bottom]
        )
...
def get_available_styles() -> Dict[str, Dict[str, Any]]:
    ...
    """
    return {name: values.copy() for name, values in STYLE_PRESETS.items()}
...
def create_custom_style(
    ...
    **kwargs: Any,
    ...
    """
    style = {"border": border, "padding": padding, "alignment": alignment}
...
def detect_border_style(text: str) -> Optional[str]:
    ...
    for name, border in BORDERS.items():
        if (
            ...
            and border["top_right"] == top_right
            and border["bottom_left"] == bottom_left
            and border["bottom_right"] == bottom_right
        ):
            return name
