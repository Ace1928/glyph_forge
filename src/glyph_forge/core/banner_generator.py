...
class BannerGenerator:
    ...
    BORDER_SETS: Dict[str, Tuple[str, str, str, str, str, str]] = {
        "single": ("┌", "┐", "┘", "└", "│", "─"),
        "double": ("╔", "╗", "╝", "╚", "║", "═"),
        "rounded": ("╭", "╮", "╯", "╰", "│", "─"),
        "heavy": ("┏", "┓", "┛", "┗", "┃", "━"),
    ...
    def __init__(
        ...
        try:
            ...
            logger.debug(
                f"BannerGenerator initialized with font='{font}', width={width}"
            )
        except Exception as e:
            logger.error(f"Failed to initialize BannerGenerator: {str(e)}")
            raise ValueError(f"Invalid font '{font}'. Error: {str(e)}") from e
    ...
    def _validate_font(self, font: str) -> None:
        ...
        if font not in self.available_fonts():
            ...
            raise ValueError(f"Font '{font}' not available. Try: {available}")
    ...
    def generate(
        ...
        cache_key = self._generate_cache_key(
            clean_text, style, padding, border, alignment, effects
        )
        ...
        if (
            self.cache_enabled and time.time() - self._last_cache_cleanup > 60
        ):  # Once per minute
    ...
    def _generate_cache_key(self, text: str, *args: object) -> str:
    ...
    def _apply_styling(
        ...
        if style in self.STYLE_PRESETS:
            style_config: Dict[str, Any] = self.STYLE_PRESETS[style].copy()
        else:
            from ..utils.glyph_utils import resolve_style

            style_config = resolve_style(style).copy()
    ...
    def _apply_border(self, text: str, border_style: str) -> str:
        ...
        if not self._unicode_supported and any(
            ord(c) > 127 for c in self.BORDER_SETS[border_style]
        ):
